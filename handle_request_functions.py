...
def get_request(site_passed):
    """
    Returns JSON content of GET request of site passed if no error has occurred, otherwise returns corresponding
    error message
    """
    try:
...
def _request_code_handler(request_response):
    """
    Returns HTTP request response passed as parameter if the response code
